...
int main(int argc, char **argv)
{
    ...
    while (ros::ok()) {
        ...
        std::cout<<"time scale is"<<t<<std::endl;
       ...
       desired_pose.setRotation(q);
        ...
        double R = 5.0;
        double timeScale = 2.0;
        std::cout<<"time for computation is"<<t/timeScale<<std::endl;
        desired_pose.setOrigin(
                        origin + tf::Vector3(R*(t/timeScale), 0.0, 10.0)
                     );
        tf::Quaternion q;
        q.setRPY(0,0,PI/4);
        desired_pose.setRotation(q);
        velocity.linear.x = R/timeScale;
        velocity.linear.y = 0;
        velocity.linear.z = 0;
